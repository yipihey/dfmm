...
\begin{itemize}
\item \textbf{D.1 2D Kelvin-Helmholtz.} Per-principal-axis noise
prediction: incompressible shear has noise on \emph{one} (compressive)
axis, not both. KH linear growth rate matches analytic.
\textbf{Falsifiable noise-structure prediction.}
...
\item \textbf{D.4 2D Zel'dovich pancake collapse.}
\textbf{Central novel test.} Pre-pancake matches Zel'dovich analytic;
$\gamma_1\to 0$ at pancake formation while $\gamma_2\sim 1$;
stochastic injection regularizes shell-crossing without spurious
$y$-direction noise; second crossing produces filament with both
$\gamma\to 0$. Compared to ColDICE 2D and PM N-body references.
...
\item \textbf{D.7 2D dust-trapping in vortices.} KH instability
with passive dust; vortex-center accumulation matches reference
codes (PencilCode, AREPO with dust); per-species diagnostics fire
selectively. \textbf{Realistic astrophysical test:
...
Multi-tracer 2D shocked turbulence; metallicity PDF and spatial
structure consistent with Lagrangian methods (SPH, AREPO);
significantly better than Eulerian methods.
...
($\Delta x^2$ midpoint-rule rate, target $\ge 3.5$). See
\verb|reference/notes_M3_prep_tierC_ic_factories.md| and
\verb|reference/notes_M3_2b_swap5_init_field.md|.
...
\item[Milestone 3-6 --- Tier-D headlines (Phases 0/1/2 DONE; 3+
ahead).] Phase 0 reactivates the off-diagonal Cholesky pair
...
\textbf{Phase 1c closes the D.1 Kelvin--Helmholtz falsifier:}
$\gamma_{\rm measured}/\gamma_{\rm DR} = 1.34$ at level $5$,
mesh-converged to $1.2\%$ between L$4$ and L$5$, $n_{\rm neg\,Jac}
= 0$ --- the per-principal-axis-noise prediction is held against
the broadband sheared base flow. \textbf{Phase 2 closes the D.4
Zel'dovich pancake test} (the central novel cosmological reference
of \S\ref{sec:tier-D}): per-axis $\gamma$ selectivity verified at
$\mathrm{std}(\gamma_1)/\mathrm{std}(\gamma_2) \approx 2.6\times
10^{14}$ at near-caustic time, $\gamma_1$ developing spatial
structure (dynamic range $4.18\times$) while $\gamma_2$ stays
uniform to round-off, and the Phase 1a strain coupling verified
inert ($\max|\beta_{\rm off}| = 0$) on this axis-aligned IC. See
\verb|reference/notes_M3_6_phase1c_D1_kh_falsifier.md| and
\verb|reference/notes_M3_6_phase2_D4_zeldovich.md|. M3-6 Phases
3 (D.7 dust trapping in vortices, requires extending
\texttt{TracerMeshHG} to 2D), 4 (D.7 close), and 5 (D.10 ISM-like
2D tracer) remain ahead.
\item[Milestone 3-7 --- 3D extension (design note in place; code
ahead).] $SO(3)$ Cholesky + Berry coupling on a 3D HG mesh; the
3D Berry stencils were verified in M3-prep
(\verb|notes_M3_prep_3D_berry_verification.md|) as the M3-7
pre-flight gate. Sub-phases M3-7a--e cover the 3D residual,
Tier-C consistency in 3D, and the 3D Zel'dovich pancake. See
\verb|reference/notes_M3_7_3d_extension.md|.
\item[Milestone 3-8 --- Tier E + GPU/MPI (ahead).] Tier-E stress
tests (\S\ref{sec:tier-E}); GPU offload of the per-cell Newton
residual and the polynomial-remap kernels; MPI distribution
across HG's hierarchical decomposition.
\item[Milestone 3-9 --- Wrap-up (ahead).] Methods-paper close-out:
final figures, two-fluid 2D content (D.6, D.8), and the paper
submission package.
\end{description}
