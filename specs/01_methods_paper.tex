...
\begin{equation}
  f(\epsilon) = \frac{1}{\sqrt\pi\,\Gamma(\lambda)}
  \frac{1}{(2\theta)^{\lambda-1/2}}|\epsilon|^{\lambda-1/2}
  K_{\lambda-1/2}\bigl(|\epsilon|/\sqrt\theta\bigr),
  \label{eq:VG}
\end{equation}
with $K_\nu$ the modified Bessel function of the second kind. Special
cases: $\lambda=1$ gives Laplace; $\lambda\to\infty$ (with $\theta$
scaled) gives Gaussian. Excess kurtosis is $3/\lambda$.
...
\subsection{Geometric overlap via r3d}

For each Lagrangian cell $T_i$ (deformed) and Eulerian cell $C_j$,
compute the polygonal overlap $T_i\cap C_j$ exactly. Using the
r3d~\cite{r3d-paper} polynomial-moment kernel, integrate polynomial
moments of the field reconstructions $\bx(\bq), \bu(\bq), L(\bq),
s(\bq), Q(\bq)$ over the overlap region. For curved-element
reconstructions (cubic or higher), use the dimension-lifting
approach: lift each curved-edge constraint to a coordinate hyperplane
in extended space (typically 5D for triangle-quad overlap with cubic
edges), where the overlap region is a polytope and r3d gives exact
moments.
...
dfmm Fig.\ 3; $\gamma$ drops by $\sim 6$ decades at caustics.
...
is sketched but not explicitly derived. The 1D reduction is verified;
...
The framework is designed for staged implementation:
\begin{description}
\item[Milestone 1.] 1D Rust implementation of the unified scheme
matching dfmm interface. Tier A regression complete. Estimated 2--3
months from this design document.
\item[Milestone 2.] 1D Tier B validation including variance-gamma
verification and tracer-exactness. Estimated 1--2 months after
Milestone 1.
\item[Milestone 3.] 2D Rust implementation. Tier C consistency tests
complete. Estimated 3--4 months after Milestone 2.
\item[Milestone 4.] 2D Tier D novelty tests. Methods-paper-ready
demonstrations. Estimated 3--4 months after Milestone 3.
\item[Milestone 5.] 2D Tier E stress tests, two-fluid extensions,
tracer applications. Estimated 2--3 months after Milestone 4.
\end{description}
Total: 12--18 months from this design document to methods-paper
submission.
