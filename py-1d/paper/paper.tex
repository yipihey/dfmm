...
The mathematical description of fluids is one of the most successful yet incomplete pillars of classical physics. At its two extremes lie the Euler and Navier-Stokes equations, which describe the collective behavior of a gas in the limit of infinite collisions, and the Vlasov and Boltzmann equations, which track the detailed evolution of a probability density function in phase space. Most astrophysical and engineering problems --- from the thinning atmosphere of a re-entering spacecraft to the diffuse, weakly collisional plasma of the intracluster medium --- reside in an intermediate, moderate-Knudsen regime where neither description is entirely accurate. Post-shock relaxation layers, the cold streams that feed galactic halos, and the multi-stream regions of cosmological dark matter structure all share a common feature: a local distribution function that is not quite Maxwellian but also not arbitrarily structured. 

The historical effort to bridge this gap began in earnest with Grad's seminal 1949 work on the kinetic theory of rarefied gases \cite{Grad1949}. By expanding the velocity distribution function in a series of Hermite polynomials, Grad derived a closed set of 13 equations that evolved not just density and velocity, but also the stress tensor and heat flux. For decades, this approach was the standard for capturing non-equilibrium behavior directly from kinetic theory, and its modern regularized descendants continue to find broad application \cite{Struchtrup2003}. However, polynomial moment systems suffered from two critical flaws: realizability (the possibility of predicting negative pressures or phase-space densities) and hyperbolicity (the requirement that waves in the fluid propagate at finite physical speeds). 

In 1996, Levermore provided a profound theoretical fix with his maximum-entropy hierarchies \cite{Levermore1996}. Instead of a polynomial expansion, Levermore proposed choosing a distribution that maximizes entropy given a set of known moments. This approach guaranteed that the resulting fluid equations would be hyperbolic and that the distribution function would remain strictly positive \cite{Groth2009}. Yet, as the field matured, it became clear that even maximum-entropy closures are often computationally expensive and can fail when the local physics --- such as the crossing of two independent streams of gas --- exceeds what a single smooth unimodal distribution can represent. The challenge with moment closures is therefore not derivation but honest deployment: the ability to know, cell by cell and step by step, whether the resulting system is still telling the truth about the underlying physics.

Parallel to the development of moment equations, the astrophysical community developed specialized tools for tracking phase-space geometry. Work by Abel, Hahn, and Kaehler \cite{Abel2012} demonstrated that the dark matter sheet in the early universe could be modeled as a thin manifold in phase space that folds and overlaps during shell crossing. The present work synthesizes this geometric insight with moment closures. Our starting point is the observation, due originally to Koh and Abel in the context of finite-volume hydrodynamics, that a Lagrangian coordinate $L_1(\vecx, t)$ --- a label identifying the initial position of the fluid element currently at $\vecx$ --- can be carried as a passive advected field without modifying the underlying hydro scheme. The gradient of $L_1$ gives the local deformation of the initial lattice and therefore the local compression or rarefaction of the fluid. The two-point correlation of $L_1$ and its Liouville-derived moments encode phase-space structure.

This paper is an attempt to instrument a moment scheme so that it reports its own closure quality as part of its output. We move from "honest deployment" to an "instrumented scheme" where closure failure is identified not as a single phenomenon, but as a set of distinct physical events, each with its own observational signature in the moment tower. We identify three such failure modes in the one-fluid problem, and two additional modes that are specific to two-fluid systems. Where all diagnostics report healthy values, the moment system is a reliable fluid description; where they fire, the local physics has exceeded what moments alone can represent, and the user is warned.
...
provided as a reproducible-science repository (\texttt{dfmm}, the
dual frame moment method, distributed under the MIT license). The scheme is implemented in
...
\bibitem{Grad1949}
Grad, H. (1949). On the kinetic theory of rarefied gases. \textit{Communications on Pure and Applied Mathematics}, 2(4), 331--407.

\bibitem{Levermore1996}
Levermore, C. D. (1996). Moment closure hierarchies for kinetic theories. \textit{Journal of Statistical Physics}, 83, 1021--1065.

\bibitem{Struchtrup2003}
Struchtrup, H., \& Torrilhon, M. (2003). Regularization of Grad's 13 moment equations: Derivation and linear analysis. \textit{Physics of Fluids}, 15(9), 2668--2680.

\bibitem{Groth2009}
Groth, C. P. T., \& McDonald, J. G. (2009). Towards physically realizable and hyperbolic moment closures for kinetic theory. \textit{Continuum Mechanics and Thermodynamics}, 21(6), 467--493.

\bibitem{Abel2012}
Abel, T., Hahn, O., \& Kaehler, R. (2012). Tracing the dark matter sheet in phase space. \textit{Monthly Notices of the Royal Astronomical Society}, 427(1), 61--76.
