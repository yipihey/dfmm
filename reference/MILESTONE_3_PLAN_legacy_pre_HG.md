# Milestone 3 implementation plan — 2D extension (LEGACY, pre-HierarchicalGrids.jl)

> **SUPERSEDED 2026-04-26** by `MILESTONE_3_PLAN.md`, which adapts the
> milestone to use [`HierarchicalGrids.jl`](https://github.com/yipihey/HierarchicalGrids.jl)
> as the dimension-generic mesh + AMR + remap substrate (eliminating
> ~80% of the infrastructure work scoped here) and to target 1D / 2D /
> **3D** in a single codebase.
>
> Kept in the repo as audit history. The methods-paper §10.7 milestone
> structure (M3 = 2D Tier C, M4 = 2D Tier D, M5 = Tier E) remains the
> authoritative validation hierarchy; the engineering re-organization
> in the new plan collapses M3 + M4 + most of M5 into a single
> dimension-generic effort.
